...
class SolutionIVP:
    ...
    def __getitem__(self, target: Union[Variable, Derivative]) -> np.ndarray:
        ...
        if isinstance(target, Variable):
            ...
            raise TypeError(
                f"Target must be Variable or Derivative, got {type(target)}"
            )

    def eval(
        self, target: Union[Variable, Derivative], tpoints: Union[float, np.ndarray]
    ) -> Union[float, np.ndarray]:
        """
        Evaluate solution at specific time points using interpolation.

        Args:
            target: Variable or Derivative to evaluate
